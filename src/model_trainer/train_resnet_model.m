...
% Use raw data instead of preprocessed
...
%% SECTION 5: Load Pre-trained Network (ResNet-50)
fprintf('\n[Step 5] Loading ResNet-50 Architecture...\n');
try
    net = resnet50;
    fprintf('   > ResNet-50 loaded successfully.\n');
catch
    error('ResNet-50 not found. Install "Deep Learning Toolbox Model for ResNet-50 Network".');
end
...
% Remove final layers (ResNet-50 specific)
...
lgraph = connectLayers(lgraph, 'avg_pool', 'new_fc1');
...
% Count layers to determine freezing threshold
...
for i = 1:numLayers
    if isprop(layers(i), 'WeightLearnRateFactor')
        if i <= freezeThreshold
            ...
            % Allow later layers to train with reduced rate
...
% Zero out diagonal to find off-diagonal confusions
...
trainingInfo.architecture = 'ResNet-50';
